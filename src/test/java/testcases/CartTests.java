...
public class CartTests extends BaseClass{
    ...
    public void testGetCartsWithLimit() 
  {
       // int limit = configReader.getIntProperty("limit");
        given()
            .pathParam("limit", 4)//we have not defined limit in config file. so passing as a path parameter.
        .when()
            .get(Routes.GET_CART_WITH_LIMIT)
        .then()
            .statusCode(200)
            .body("size()", lessThanOrEqualTo(4)); // Validate that the response size is within the limit
    }
